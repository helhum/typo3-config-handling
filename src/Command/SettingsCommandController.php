...
class SettingsCommandController extends CommandController
{
    ...
    public function dumpCommand($noDev = false)
    {
        ...
        if ($noDev) {
            if (!$this->isAllowedToGenerate($additionalConfigurationFile) || !$this->isAllowedToGenerate($localConfigurationFile)) {
                $this->outputLine('<error>TYPO3 configuration files are not auto generated.</error>');
                $this->outputLine('<error>Please verify, use settings:extract command and delete them afterwards.</error>');
                $this->quit(1);
            }
        ...
        $this->configurationManager->writeAdditionalConfiguration(
            [
                ...
                RootConfig::getInitConfigFileContent($config),
            ]
        );
    ...
    private function isAllowedToGenerate(string $file): bool
    {
        return !file_exists($file) || $this->isAutoGenerated($file);
    }
}
